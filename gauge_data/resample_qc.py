...
def time_elapsed(datetimes):
    ...
    time_elapsed_s = datetime_series.diff().dt.total_seconds()
    # convert to time elapsed in hours
    time_elapsed_h = time_elapsed_s / 3600
    df = pd.DataFrame(
        data={
            'time_elapsed': time_elapsed_h,
            'date_time': datetimes})
...
def resample(data_raw, datetimes):
    ...
    for i in range(len(indices))[
        ...
        if (df['time_elapsed'].loc[indices[i]] >
                1):                # find intervals of >1 hour
            # check if there is precip recorded during the interval
            if precip.loc[indices[i]] != 0:
                ...
                # divide the original amount recorded by the number of new 5
                # min periods
                old_value = temp_col.iloc[-1]
                count = len(temp_col[1:])
                value_dist = old_value / count
                # populate temporary column with even precip intensities that
                # sum to original recorded amount
                temp_col.iloc[1:] = value_dist

                # place the new 5 minute rainfall intervals into the
                # precipitation time series
                if indices[i] == indices[-1]:
                    precip = pd.concat(
                        (precip.loc[:indices_new[i_new - 1]], temp_col[1:]))
                else:
                    precip = pd.concat(
                        (precip.loc[:indices_new[i_new - 1]], temp_col[1:], precip.loc[indices_new[i_new + 1]:]))
...
def get_atlas14(which='mean', folderpath=None):
    ...
    if which == 'mean':
        ...
        raise KeyWordError(
            "'which' keyword must be one of 'upper', 'lower', or 'mean'")
...
def quality_control(
    ...
    if (df['time_elapsed'].dropna() > 1).all():
        if (df['precip_data'].dropna() == 0).all() == False:
            # station fails QC if readings are longer than hourly
            qc_flag = 1

    # define data, copies, and indices
    precip = df['precip_data']
    # copy of original data
    precip_orig = precip.copy()

    # set up precip observations to use for QC process
    # copy of original data without NaNs
    precip_qc = precip.copy().dropna()
    # times associated with gauge readings
    indices = precip_qc.index

    # copy of qc data
    precip_qc_copy = precip_qc.copy()
    # accumulated precip data
    precip_qc_sum = precip_qc_copy.cumsum()
    precip_qc_sum_resample = precip_qc_sum.resample(
        '1H').ffill()   # accumulated data resampled to hourly
    precip_qc_resample = precip_qc_sum_resample.diff().fillna(
        0)    # interval data at hourly resolution

    # check for "negative" rainfall values
    if (precip_orig < 0).any():
        qc_flag = 2
    ...
    if PFDS:
        ...
        for i in range(len(indices))[1:]:
            ...
            # find the atlas 14 duration that best matches the duration of the
            # reading
            duration_key = min(
                atlas14_durations.items(),
                key=lambda x: abs(
                    spike_duration - x[1][0]))

            # find the 1000 year return interval intensity associated with the
            # duration
            atlas14_path = f'{atlas14_root_mean}/{
                duration_key[0]}_pds/sw1000yr{
                duration_key[1][1]}a.nc'
            atlas14_da = xr.open_dataarray(atlas14_path)
            atlas14_int = atlas14_da.sel(
                lat=lat, lon=lon, method='nearest').values[0]

            # find the upper confidence interval of 1000 year return interval
            # intensity associated with the duration
            atlas14_path_upper = f'{atlas14_root_upper}/{
                duration_key[0]}_pds/sw1000yr{
                duration_key[1][1]}au.nc'
            atlas14_da_upper = xr.open_dataarray(atlas14_path_upper)
            atlas14_int_upper = atlas14_da_upper.sel(
                lat=lat, lon=lon, method='nearest').values[0]

            # check if intensity is greater than 1.5x the upper level of 1000
            # return interval intensity
            if rainspike > 1.5 * atlas14_int_upper:
                qc_flag = 3
                break

            # check if intensity recorded rainfall is isolated (ie., no precip
            # leading up to or following)
            elif rainspike > atlas14_int:
                # search 5 readings before and 5 readings after
                # station is flagged if the readings either before or after are
                # less than 0.1x the recorded high intensity
                cond_pre = (precip_qc[indices[i - 5:i]]
                            < (0.1 * rainspike)).all()
                cond_post = (precip_qc[indices[i + 1:i + 6]]
                             < (0.1 * rainspike)).all()
                if (cond_pre) | (cond_post):
                    qc_flag = 4
                    break
        ...
        if qc_flag == 0:
            ...
            for key in list(atlas14_durations.keys())[4:]:
                ...
                time_sum = precip_qc_resample.rolling(
                    atlas14_durations[key][0]).sum().max()
                # find the upper confidence interval of 1000 year return
                # interval intensity associated with the duration
                atlas14_path_upper = f'{atlas14_root_upper}/{key}_pds/sw1000yr{
                    atlas14_durations[key][1]}au.nc'
                atlas14_da_upper = xr.open_dataarray(atlas14_path_upper)
                atlas14_int_upper = atlas14_da_upper.sel(
                    lat=lat, lon=lon, method='nearest').values[0]

                # check if intensity is greater than 1.5x the upper level of
                # 1000 return interval intensity
